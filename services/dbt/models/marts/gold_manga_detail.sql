...
artist_names as (
    select
        ma.source_manga_id,
        jsonb_agg(distinct a.name) filter (where a.name is not null) as artist_names
    from {{ ref('stg_manga_artist') }} ma
    join {{ ref('stg_author') }} a on a.source_author_id = ma.source_author_id
    group by ma.source_manga_id
),
tag_names as (
...
select
    ...
    m.description,
    m.original_language,
    m.available_translated_languages,
    m.status,
    m.year,
    m.content_rating,
    ...
    coalesce(ar.artist_names, '[]'::jsonb) as artist_names,
    c.cover_file_name,
    m.created_at,
    m.updated_at
